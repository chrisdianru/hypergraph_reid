...
from utils import AverageMeter, Logger, save_checkpoint, build_adj, build_adj_full, build_adj_full_full, WarmupMultiStepLR
...
parser.add_argument('--nheads', default=8, type=int, help='number of attention heads for GAT')
parser.add_argument('--concat', default=False, type=bool, help='')
parser.add_argument('--part1', default=4, type=int, help='')
parser.add_argument('--part2', default=8, type=int, help='')
parser.add_argument('--part3', default=2, type=int, help='')
...
def main():
    ...
    if args.arch=='resnet503d':
        ...
        model = models.init_model(name=args.arch, pool_size=8, input_shape=2048, n_classes=dataset.num_train_pids, loss={'xent', 'htri'})
...
#def train(model, criterion_xent, criterion_htri, optimizer, trainloader, use_gpu, adj1, adj2, adj3):
def train(model, criterion_xent, criterion_htri, optimizer, trainloader, use_gpu):
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        #outputs, features = model(imgs, adj1, adj2, adj3)
        ...
        if args.htri_only:
            ...
            #xent_loss = torch.sum(torch.cat([criterion_xent(logits, pids) for logits in outputs]))
            xent_loss = torch.sum(torch.stack([criterion_xent(logits, pids) for logits in outputs], dim=0))
            #htri_loss = torch.sum(torch.stack([criterion_htri(feats, pids) for feats in features]))
        ...
        loss.backward()
        optimizer.step()
        #losses.update(loss.data[0].item(), pids.size(0))
        losses.update(loss.data.item(), pids.size(0))

        if (batch_idx+1) % args.print_freq == 0:
            print("Batch {}/{}\t Loss {:.6f} ({:.6f})".format(batch_idx+1, len(trainloader), losses.val, losses.avg))

#def test(model, queryloader, galleryloader, pool, use_gpu, adj1, adj2, adj3, ranks=[1, 5, 10, 20]):
